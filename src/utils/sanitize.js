...
const transformFontTag = (tagName, attribs) => {
  return {
    tagName,
    attribs: {
      ...attribs,
...
const transformSpanTag = (tagName, attribs) => {
  return {
    tagName,
    attribs: {
      ...attribs,
...
const transformATag = (tagName, attribs) => {
...
const transformImgTag = (tagName, attribs) => {
  const { src } = attribs;
...
export const sanitizeCustomHtml = (matrixClient, body) => {
  mx = matrixClient;
...
export const sanitizeText = (body) => {
  const tagsToReplace = {
    '&': '&amp;',
    '<': '&lt;',
    '>': '&gt;',
