...
import { markdown, plain } from 'utils/markdown';
import { sanitizeText } from 'utils/sanitize';

const blurhashField = 'xyz.amorgan.blurhash';

const encodeBlurhash = (img) => {
...
const getVideoThumbnail = (video, width, height, mimeType) => {
...
class RoomsInput extends EventEmitter {
  ...
  cancelAttachment(roomId) {
    ...
    this.roomIdToInput.set(roomId, input);
    this.emit('ATTACHMENT_CANCELED', roomId);
  }
  ...
  async sendFile(roomId, file) {
    ...
    if (fileType === 'image') {
      ...
      try {
        ...
        this.emit('FILE_UPLOAD_CANCELED', roomId);
        return;
      }
    ...
    try {
      uploadData = await this.uploadFile(roomId, file, (data) => {
        ...
        this.emit('UPLOAD_PROGRESS_CHANGES', roomId, data);
      });
      this.emit('FILE_UPLOADED', roomId);
    } catch (e) {
      this.emit('FILE_UPLOAD_CANCELED', roomId);
      return;
    }
