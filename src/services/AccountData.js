...
class AccountData extends EventEmitter {
  ...
  _getAccountData() {
    return this.matrixClient.getAccountData(cons.IN_BIPLES_SPACES)?.getContent() || {};
  }
  ...
  _updateSpaceShortcutData(shortcutList) {
    ...
    this.matrixClient.setAccountData(cons.IN_BIPLES_SPACES, spaceContent);
  }
  ...
  _listenEvents() {
    this.matrixClient.on('accountData', (event) => {
      if (event.getType() !== cons.IN_BIPLES_SPACES) return;
      this._populateSpaceShortcut();
